...
build-linux: linux/arch/arm64/boot/Image

linux/.config: linux aarch64-none-elf-gcc
	$(Q)make -C linux ARCH=arm64 CROSS_COMPILE=aarch64-none-elf- defconfig

linux/arch/arm64/boot/Image: linux linux/.config aarch64-none-elf-gcc
	$(Q)make -C linux \
	    -j$(_NPROCESSORS_ONLN) \
	    ARCH=arm64 \
...
linux/usr/gen_init_cpio: linux linux/.config
	$(Q)make -C linux ARCH=arm64 usr/gen_init_cpio

linux/scripts/dtc/dtc: linux linux/.config aarch64-none-elf-gcc
...
optee_os/out/arm32-plat-vexpress/core/tee.bin:
	$(ECHO) '  BUILD   optee_os'
	$(Q)make -C optee_os \
	    -j$(_NPROCESSORS_ONLN) \
...
build-optee-linuxdriver: optee_linuxdriver/optee.ko

optee_linuxdriver/optee.ko: linux/.config aarch64-linux-gnu-gcc
...
build-dtb: optee_linuxdriver/dtb/fvp-foundation-gicv2-psci.dtb

optee_linuxdriver/dtb/fvp-foundation-gicv2-psci.dtb: optee_linuxdriver/fdts/fvp-foundation-gicv2-psci.dts linux/scripts/dtc/dtc
	$(ECHO) '  GEN     $@'
...
build-uefi: edk2/Build/ArmVExpress-FVP-AArch64/RELEASE_GCC49/FV/FVP_AARCH64_EFI.fd

edk2/Build/ArmVExpress-FVP-AArch64/RELEASE_GCC49/FV/FVP_AARCH64_EFI.fd: edk2 aarch64-none-elf-gcc
	$(ECHO) '  BUILD   edk2'
...
build-arm-tf-fip: arm-trusted-firmware/build/fvp/debug/fip.bin

arm-trusted-firmware/build/fvp/debug/fip.bin : aarch64-none-elf-gcc optee_os/out/arm32-plat-vexpress/core/tee.bin edk2/Build/ArmVExpress-FVP-AArch64/RELEASE_GCC49/FV/FVP_AARCH64_EFI.fd
	$(ECHO) '  BUILD   $@'
...
build-arm-tf-bl1: aarch64-none-elf-gcc
	$(ECHO) '  BUILD   $@'
	$(Q)export CFLAGS="-O0 -gdwarf-2" ; \
	    make -C arm-trusted-firmware \
	    CROSS_COMPILE="$(CCACHE)aarch64-none-elf-" \
...
run/filesystem.cpio.gz: linux/usr/gen_init_cpio build-optee-client build-optee-linuxdriver
